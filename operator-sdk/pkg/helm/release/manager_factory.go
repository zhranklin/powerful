...
func (f managerFactory) NewManager(cr *unstructured.Unstructured, overrideValues map[string]string) (Manager, error) {
	...
	if overlayYaml, err := ioutil.ReadFile("helm-charts/powerful/values-overlay.yaml"); err != nil {
		println("###########Failed to get overlay file, skip...")
	} else if overlayJson, err := yaml.YAMLToJSON(overlayYaml); err != nil {
		return nil, fmt.Errorf("failed to parse overlay file")
	} else if overlayValues := &map[string]interface{}{}; json.Unmarshal(overlayJson, overlayValues) != nil {
		return nil, fmt.Errorf("failed to parse overlay file")
	} else {
		crValues = mergeMaps(*overlayValues, crValues)
	}
