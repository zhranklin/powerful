 ...
 */
@GrpcService
@ConditionalOnProperty(name = "framew.application.name", havingValue = "grpc-c")
public class GrpcCService extends GrpcPowerfulService {
    private static Logger logger = LoggerFactory.getLogger(GrpcCService.class);

    private ObjectMapper objectMapper = new ObjectMapper();

    @PostConstruct
    private void init() {
        logger.info("init grpc-c service ...");
        objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
    }

    public GrpcCService(StringRenderer stringRenderer, ApplicationContext applicationContext) {
    ...
    public void echo(EchoNum request, StreamObserver<Reply> responseObserver) {
        logger.info("in GrpcCService num: " + request.getNum());
        try {
